...
public final class CalloutFactory {
  ...
  static final ICallout health_callout = payload -> new HealthResponse() {
    {
      this.timestamp = Instant.now().toString();
      this.system = new TreeMap<String, Object>() {
        {
  ...
  static final ICallout ping_callout = payload -> new PingResponse() {
    {
      this.timestamp = Instant.now().toString();
      this.ping = PingInfo.create();
    }
  };
...
//    logger.log(Log.Level.debug,"implementDefaultCallout " + endpoint.toString());
  ...
  public CalloutFactory implementSpecificCallout(final Endpoints endpoint, final ICallout callout){
